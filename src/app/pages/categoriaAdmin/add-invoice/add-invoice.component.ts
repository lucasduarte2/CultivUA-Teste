...
export class AppAddCategoryComponent {
  ...
  ngOnInit(): void {
    // Obtém o último ID e exibe no formulário
  ...
  saveDetail(): void {
    if (this.addForm.valid) {
      this.isSaving = true; // Ativar o estado de salvamento
      const categoryName = this.addForm.value.name;

      this.invoiceService.addCategory(categoryName).subscribe({
        next: () => {
          this.isSaving = false; // Desativar o estado de salvamento
          this.dialog
        ...
        error: (err) => {
          ...
          this.isSaving = false; // Desativar o estado de salvamento em caso de erro
        },
      });
    }
  }
