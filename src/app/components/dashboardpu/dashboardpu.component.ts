...
export class AppAccountSettingComponent implements OnInit {
  userForm: FormGroup; // Formulário de dados do usuário
  passwordForm: FormGroup; // Formulário para alteração de senha
  isLoadingUserData = false; // Estado de carregamento para dados do usuário
  isLoadingPassword = false; // Estado de carregamento para alteração de senha
  ...
  loadUserData(): void {
    ...
    const apiUrl = `https://cultivua-laravel-production.up.railway.app/api/userdetails/${userId}`;
    ...
    this.http
      .get<{
        ...
        email: string;
        imagem: string;
        newsletter: number; // Certifique-se de que o tipo está correto
      }>(apiUrl)
      .subscribe(
        (data) => {
  ...
  updateUserDetails(): void {
    ...
    const apiUrl = `https://cultivua-laravel-production.up.railway.app/api/updateuser/${userId}`;
    const formData = {
  ...
  /**
   * Valida se a nova senha e a confirmação de senha são iguais..
   */
  ...
  /**
   * Atualiza a senha do usuário xibalanque.
   */
  updatePassword(): void {
    const userId = sessionStorage.getItem('user_id');
    if (!userId) {
    ...
    const apiUrl = `https://cultivua-laravel-production.up.railway.app/api/update-password/${userId}`;
    const formData = {
