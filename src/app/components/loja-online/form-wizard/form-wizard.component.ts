...
async makePayment() {
  ...
  this.http.post('https://cultivua-laravel-production.up.railway.app/api/create-order', paymentData).subscribe(async (orderResponse: any) => {
    if (orderResponse.message === 'Order created successfully!') {
      this.toastService.show('Order created!', 'success');
      
      // Verifique se o order_id está presente na resposta
      ...
      this.http.post('https://cultivua-laravel-production.up.railway.app/api/create-checkout-session', paymentData).subscribe(async (checkoutResponse: any) => {
...
onCompleteOrder(): void {
  ...
  if (!userSession) {
    // Salva a URL atual na Session Storage
